...
'''
This is the funtion to see the visualizations in the Detection only model
'''
def draw_detections_online(frame, detections, show_all_detections=True):
    """Draws detections and labels"""
    ...
    for i, obj in enumerate(detections):
        ...
        '''
        Here we generate a randome id just to compute the color for the visulaization 
        And we comment the id comming from the tracking because in realtime we are not tracking 
        And we also replacing the ID checking replacing and display it on the video
        '''
...
def visualize_multicam_detections(frames,mode, all_objects, fps='', show_all_detections=True,
    ...
    for i, (frame, objects) in enumerate(zip(frames, all_objects)):
        ...
        if mode == 'online':
            draw_detections_online(frame, objects, show_all_detections)
        if vis is not None:
            if stack_frames == 'vertical':
