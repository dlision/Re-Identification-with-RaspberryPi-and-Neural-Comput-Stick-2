...
class Detector(DetectorInterface):
    ...
    '''
    Here we add an argument of frame shape to get the size of frame to save the video
    '''
    def __init__(self, ie, model_path, trg_classes, conf=.6,
                 device='CPU', ext_path='', max_num_frames=1,frame_shape=[]):
        ...
        '''
        Here we create video writer objec according to the number of input cameras
        And get the hight and width of the camera to save the video
        '''
        self.frame_height = frame_shape[0]
        self.frame_width = frame_shape[1]
        self.out = [[] for _ in range(self.max_num_frames)]      
        for i in range(0,self.max_num_frames):
            self.out[i] = cv2.VideoWriter('video'+str(i)+'.avi', fourcc, 25, (self.frame_width,self.frame_height))
    ...
    def run_async(self, frames, index):
        ...
        for i in range(len(frames)):
            ...
            '''
            Here we are saving video for later use in ReID model
            '''
            self.out[i].write(frames[i])
